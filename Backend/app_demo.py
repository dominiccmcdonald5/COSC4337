...
def health_check():
    ...
    return jsonify({'status': 'healthy', 'message': 'Biodiversity Analysis API is running (Demo Mode)'})

@app.route('/', methods=['GET'])
def root():
    """Root endpoint"""
    return jsonify({
        'message': 'Biodiversity Analysis API - Demo Mode',
        'note': 'This is a simplified demo version. Upload any audio file to see the analysis!',
        'endpoints': {
...
if __name__ == '__main__':
    logger.info("Starting Biodiversity Analysis API (Demo Mode)...")
    logger.info("Note: This is a simplified version for demonstration purposes.")
